...
class QuizRepository extends ServiceEntityRepository
{
     ...
     * @return Quiz[]
     */
    public function findForContentForm(?int $creatorId = null, ?int $courseId = null, int $limit = 200): array
    {
        $qb = $this->createQueryBuilder('q');

        if ($creatorId !== null) {
            $qb->andWhere('q.idCreateur = :creatorId')
                ->setParameter('creatorId', $creatorId);
        }

        if ($courseId !== null && $courseId > 0) {
            $qb->andWhere('q.idCours = :courseId OR q.idCours IS NULL')
